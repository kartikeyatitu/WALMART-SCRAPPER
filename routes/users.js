...
router.post('/password/change', isAuthenticatedUser,(req, res)=> {
  if(req.body.password !== req.body.confirmpassword) {
      req.flash('error_msg', "Password don't match. Type again!");
      return res.redirect('/password/change');
  } //if both fields are same  then save new password in database
...
//rotes for frgort passsword and sending mail using nodemailer

// Routes to handle forgot password
router.post('/forgot', (req, res, next) => {
  let recoveryPassword = '';
  async.waterfall([
    (done) => { //generate unique token for our user
    ...
    (token, user) => {
      ...
      let mailOptions = {
        to: user.email,
        from: ' Titu titutitu259@gmail.com',
